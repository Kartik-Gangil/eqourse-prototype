...
const faqsData = JSON.parse(fs.readFileSync('extracted_faqs.json', 'utf8'));

// Helper to parse the JS array strings from extracted JSON into real JS objects
// Since they are strings like "const faqs = [ ... ];", we can strip "const faqs =" and evaluate.
function parseFaqString(str) {
  try {
    let toEval = str.replace(/const faqs.*?(?=\[)/s, '');
    // remove trailing semicolon
    toEval = toEval.replace(/;$/, '');
    // Use eval safely in this isolated context to convert the object string
    return eval(toEval);
  } catch(e) {
...
let contentServicesFaqs = [];
let aiDataFaqs = [];
let freePilotFaqs = [];

// Group the files
const csFiles = ['AccessibilityPage.tsx', 'CustomElearningPage.tsx', 'EditorialPublishingPage.tsx', 'ExamPrepPage.tsx', 'LearningSolutionsPage.tsx', 'LocalizationPage.tsx', 'ContentServicesOverviewPage.tsx', 'SMEPage.tsx', 'TalentAssessmentPage.tsx', 'TechSolutionsPage.tsx'];
const aiFiles = ['AnnotationPage.tsx', 'CleaningPage.tsx', 'DataCollectionPage.tsx', 'ModelTestingPage.tsx', 'OverviewPage.tsx'];

// Existing FAQs from FAQsAccordion.tsx (manually copied to avoid losing them)
const existingCs = [
  {
...
contentServicesFaqs.push(...existingCs);
aiDataFaqs.push(...existingAi);

const addedQuestions = new Set();
existingCs.forEach(f => addedQuestions.add(f.question));
existingAi.forEach(f => addedQuestions.add(f.question));

Object.keys(faqsData).forEach(file => {
  const parsed = parseFaqString(faqsData[file]);
  if (parsed && Array.isArray(parsed)) {
    parsed.forEach(f => {
      // Fix encoding issues from parsed output like "client?Ts"
      let answer = f.answer.replace(/\?T/g, "'");
      answer = answer.replace(/\?"/g, "-");
      if (!addedQuestions.has(f.question)) {
        addedQuestions.add(f.question);
        const faqObj = { question: f.question, answer: answer };
        if (file === 'FreePilotFAQs.tsx') {
          freePilotFaqs.push(faqObj);
        } else if (csFiles.includes(file)) {
          contentServicesFaqs.push(faqObj);
        } else if (aiFiles.includes(file)) {
          aiDataFaqs.push(faqObj);
        }
      }
    });
  }
...
import { ChevronDown, GraduationCap, Brain, ExternalLink, Rocket } from "lucide-react";
...
const contentServicesFaqs: FAQ[] = ${serializeFaqs(contentServicesFaqs)};

const aiDataFaqs: FAQ[] = ${serializeFaqs(aiDataFaqs)};

const freePilotFaqs: FAQ[] = ${serializeFaqs(freePilotFaqs)};

const allFaqs = [...contentServicesFaqs, ...aiDataFaqs, ...freePilotFaqs];

const FAQItem = ({ faq, isOpen, toggle, type }: { faq: FAQ, isOpen: boolean, toggle: () => void, type: 'content-services' | 'ai' | 'pilot' }) => {
  const getColors = () => {
    if (type === 'content-services') return { border: 'border-teal-500/50', bg: 'bg-teal-500', icon: GraduationCap };
    if (type === 'ai') return { border: 'border-indigo-500/50', bg: 'bg-indigo-600', icon: Brain };
  ...
  return (
    <div className={\`mb-4 border border-border/50 rounded-2xl overflow-hidden transition-all duration-300 \${isOpen ? \`\${colors.border} shadow-md bg-secondary/30\` : 'hover:border-primary/30 glass'}\`}>
      <button 
        onClick={toggle} 
        className="w-full px-6 py-5 flex items-center justify-between gap-4 text-left focus:outline-none"
      >
        <div className="flex items-center gap-4">
          <div className={\`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 transition-colors \${isOpen ? \`\${colors.bg} text-white\` : 'bg-primary/10 text-primary'}\`}>
...
fs.writeFileSync('src/components/faqs/FAQsAccordion.tsx', finalCode);
console.log("Successfully generated FAQsAccordion.tsx with all aggregated FAQs!");
