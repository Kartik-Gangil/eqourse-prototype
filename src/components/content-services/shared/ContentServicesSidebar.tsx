...
import { contentServicesCategories, type ContentServicesCategory } from "./contentServicesNavData";

/* ─── Sidebar ─── */
const ContentServicesSidebar = () => {
  const { pathname } = useLocation();
  const [collapsed, setCollapsed] = useState(false);

  // Auto-expand the category that owns the current page
  const findActiveCategoryIndex = () => {
    return contentServicesCategories.findIndex(
...
const MobileSidebarSheet = ({ pathname }: { pathname: string }) => {
  ...
  const [expandedIdx, setExpandedIdx] = useState<number | null>(() =>
    contentServicesCategories.findIndex((cat) => pathname === cat.href || pathname.startsWith(cat.href + "/"))
  );

  useEffect(() => {
    const idx = contentServicesCategories.findIndex(
      (cat) => pathname === cat.href || pathname.startsWith(cat.href + "/")
    );
    if (idx >= 0) setExpandedIdx(idx);
  }, [pathname]);
        ...
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <div className="relative w-[300px] max-w-[80vw] bg-card border-r border-border/50 shadow-elevated h-full flex flex-col animate-slide-up">
            <div className="flex items-center justify-between px-4 py-3 border-b border-border/30">
              <span className="text-sm font-bold text-foreground">Content Services</span>
              <button
...
export default ContentServicesSidebar;
