...
import {
  contentServicesSamples,
  type ContentServicesSample,
} from "../contentServicesSamplesData";

interface Props {
  sample: ContentServicesSample;
}

const ContentServicesLandingBody = ({ sample }: Props) => {
  const accent = `hsl(${sample.accentHsl})`;
  const isTextLanding = sample.kind === "text-landing";
  const subSamples = contentServicesSamples.filter((s) =>
...
export default ContentServicesLandingBody;
