...
export const SecurityRelatedWhy = () => {
  ...
  return <>
    ...
    <section className="bg-background py-20 md:py-24"><div className="container mx-auto px-4"><SectionHeader label="Connected Services" title="Continue Through the" gradientText="AI Data Pipeline"/><div className="mx-auto mt-12 grid max-w-6xl gap-px overflow-hidden rounded-[2rem] border border-border bg-border sm:grid-cols-2 lg:grid-cols-3">{[["Video Annotation","/ai-data-services/annotation-labeling/video-annotation"],["Image Data Collection","/ai-data-services/data-collection/image-data-collection"],["Data Cleaning & Validation","/ai-data-services/cleaning-validation"],["AI Model Testing","/ai-data-services/model-testing"],["All Annotation Services","/ai-data-services/annotation-labeling"],["Computer Vision Samples","/ai-data-samples/computer-vision"]].map(([label,href])=><Link key={label} to={href} className="group flex items-center justify-between bg-card p-6 font-heading font-bold hover:bg-primary/[.05]">{label}<ArrowRight className="h-4 w-4 text-primary transition-transform group-hover:translate-x-1"/></Link>)}</div><p className="mx-auto mt-7 max-w-4xl text-center text-xs leading-6 text-muted-foreground">3D point-cloud & LiDAR annotation and Document & OCR annotation have separate planned pages and remain unlinked until live.</p></div></section>
