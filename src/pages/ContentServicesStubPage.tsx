...
const ContentServicesStubPage = () => {
  const { pathname } = useLocation();
  const title = pathname.split("/").pop()?.replace(/-/g, " ").replace(/\b\w/g, l => l.toUpperCase()) || "Page Not Found";

  return (
    <ContentServicesLayout breadcrumbs={[{ label: "Content Services", href: "/content-services" }, { label: title }]}>
      <SEOHead
        title={`${title} | eQOURSE`}
      ...
      <div className="min-h-[70vh] flex items-center justify-center bg-background px-4">
        <div className="text-center max-w-md w-full p-8 rounded-3xl neon-card border border-border/50 bg-card">
          ...
          >
            <ArrowLeft className="w-4 h-4" /> Back to Content Service
          </Link>
        </div>
      </div>
...
export default ContentServicesStubPage;
